---
title: "Eva María Corregidor Luna"
image: "./assets/img/headshot_1.jpeg"
page-layout: full
toc: false
about:
  id: hero-heading
  template: trestles
  image-width: 15em
  image-shape: round
  links:
    - text: "{{< fa envelope size=2xl >}}"
      href: mailto:eva.corregidor@rutgers.edu
    - text: "{{< fa brands github size=2xl >}}"
      href: https://github.com/emcorregidor
#    - text: "{{< ai orcid size=2xl >}}"
#      href: https://orcid.org/0000-0001-8735-9910
#    - text: "{{< ai google-scholar size=2xl >}}"
#      href: https://scholar.google.com/citations?user=6sd7cVAAAAAJ&amp;hl
#    - text: "{{< ai researchgate size=2xl >}}"
#      href: https://www.researchgate.net/profile/Joseph_Casillas
engine: knitr
---

```{css}
/*| echo: false */

.about-entity {
  padding-top: 100px;
}

.about-link {
  color: #cc0033 !important;
  padding: 10px !important;
  border: 0 !important;
  border-radius: 5px !important;
  background-color: transparent !important;
}

.about-link:hover {
  background-color: #717171 !important;

}

h1 {
  line-height: 1.1;
  margin-bottom: 4px;
  font-size: 1.5em;
  color: #515151 !important;
}

h3 {
  color: #5F6A72;
  margin-bottom: 8px;
  margin-top: 0px;
}

h4 {
  color: #cc0033;
  text-transform: none;
  font-size: 18px;
  margin-top: 0px;
}

.myfooter {
  color: #fff;
  background-color: #fff;
  font-size: 0;
}

.myfooter img {
  width: 0;
}

```

```{r}
#| label: printing-fuctions
#| echo: false

wrap_li <- function(text = "text", fa = "fas fa-graduation-cap") {
  li <- paste0("<li><i class='fa-li ", fa, "'></i>", text, "</li>\n")
  return(li)
}

print_list <- function(text = "text", fa = "fas fa-graduation-cap") {
  open_ul  <- "<ul class='ul-edu fa-ul'>"
  close_ul <- "</ul>"
  items    <- wrap_li(text, fa)

  cat(paste0(open_ul, "\n"), items, paste0("\n", close_ul))
}

```

### Ph.D. Student

#### Rutgers University

<p align="justify">
To be added...
</p>


<br>

:::: {style="display: grid; grid-template-columns: 1fr 1fr; grid-column-gap: 10px;"}

::: {}

##### Interests

```{r}
#| label: interests
#| echo: false
#| results: 'asis'

ul <- "- "
elements <- c(
 "Language and emotions",
 "Cognition",
 "Bilingualism",
 "Health"
)
glue::glue("{ul}{elements}")
```

:::

::: {}

##### Education

```{r}
#| label: print-ed
#| echo: false
#| results: 'asis'
print_list(
  text = c(
    "PhD in Bilingualism and Second Language Acquisition, expected 2029 <br>Rutgers University", 
    "MA in Spanish, 2024 <br>San Diego State University", 
    "BA in Modern Languages and Translation, 2021 <br>Universidad de Alcalá de Henares"
    )
  )
```

:::

::::
